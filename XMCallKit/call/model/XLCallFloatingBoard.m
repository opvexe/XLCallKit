...
@interface XLCallFloatingBoard () <XLCallSessionDelegate>
@property(nonatomic, strong) NSTimer *activeTimer;
@property(nonatomic, copy) void (^touchedBlock)(XLCallSession *callSession);
@property(nonatomic, strong) CTCallCenter *callCenter;
@end

static XLCallFloatingBoard *staticBoard = nil;
...
- (void)initBoard {
    if (self.callSession.callStatus == XLCallHangup) {
...
- (void)updateActiveTimer {
    ...
    if (self.callSession.callStatus == XLCallActive && ![self isVideoViewEnabledSession]) {
        [self.floatingButton setTitle:[XLCallVideoUtility getTalkTimeStringForTime:sec] forState:UIControlStateNormal];
...
- (void)updateBoard {
    ...
    if ([self isVideoViewEnabledSession]) {
        if (self.callSession.callStatus == XLCallActive) {
            ...
            videoStopTips.text = @"已经挂断";
            videoStopTips.textColor = UIColorFromRGB(0x0195ff);
        ...
        if (self.callSession.callStatus == XLCallActive) {
            ...
            [self.floatingButton setTitle:@"已经挂断"
...
- (UIWindow *)window {
    if (!_window) {
        ...
        _window.layer.borderColor = [UIColorFromRGB(0x0195ff) CGColor];
...
- (UIButton *)floatingButton {
    if (!_floatingButton) {
        ...
        if (self.callSession.mediaType == XLCallMediaAudio) {
            [_floatingButton setImage:[UIImage imageNamed:@"audio_min.png"]
                             forState:UIControlStateNormal];
        } else {
            [_floatingButton setImage:[UIImage imageNamed:@"video_min.png"]
                             forState:UIControlStateNormal];
        }
...
- (void)layoutTextUnderImageButton:(UIButton *)button {
    ...
    [button setTitleColor:UIColorFromRGB(0x0195ff) forState:UIControlStateNormal];
    
    button.titleEdgeInsets = UIEdgeInsetsMake(0, -button.imageView.frame.size.width,
                                              -button.imageView.frame.size.height - LSWMCallInsideMargin / 2, 0);
    button.imageEdgeInsets = UIEdgeInsetsMake(-button.titleLabel.intrinsicContentSize.height - LSWMCallInsideMargin / 2,
                                              0, 0, -button.titleLabel.intrinsicContentSize.width);
}

- (BOOL)isVideoViewEnabledSession {
    if (self.callSession.mediaType == XLCallMediaVideo && !self.callSession.isMultiCall) {
        return YES;
    } else {
        return NO;
    }
...
- (void)callDidDisconnect {
    [self updateBoard];
    [self performSelector:@selector(clearCallFloatingBoard) withObject:nil afterDelay:2];
    [XLCallVideoUtility clearScreenForceOnStatus];
